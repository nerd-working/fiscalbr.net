...
namespace FiscalBr.Common.Sped
{
    public static class EscreverCamposSped
    {
        ...
        static Dictionary<string, SpedCamposAttribute[]> SpedCamposAttributeRepository = new Dictionary<string, SpedCamposAttribute[]>();
        ...
        private static bool ExisteAtributoPropriedadeParaVersao(PropertyInfo prop, int versao)
        {
            ...
            if (!SpedCamposAttributeRepository.ContainsKey(propName))
                SpedCamposAttributeRepository.Add(propName, (SpedCamposAttribute[])Attribute.GetCustomAttributes(prop, typeof(SpedCamposAttribute), false));

            SpedCamposAttribute[] attrs = SpedCamposAttributeRepository[propName];
